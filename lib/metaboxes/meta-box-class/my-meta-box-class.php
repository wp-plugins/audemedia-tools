...
class AT_Meta_Box {
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    echo '<input type="button" class="button" id="add-'.$field['id'].'" value="'.__('Add','mmb').'" style="clear:both; display:block">';
    ...
    echo '</table><input type="button" class="button" style="display:none" id="remove-'.$field['id'].'" value="'.__('Remove','mmb').'"></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
  ...
  public function show_field_image( $field, $meta ) {
    ...
    echo "<span class='simplePanelImagePreview'><div class='crop'><img {$PreviewStyle} src='{$value['url']}'></div><br/></span>";
    echo "<input type='hidden' name='{$name}[id]' value='{$value['id']}'/>";
    echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
    if ($has_image)
